...
module.exports = async (client, interaction) => {
    ...
    if (!client.constants.DOCUMENTATION_SOURCES.includes(source)) return message.channel.send('That source doesnt exist!')
    if (source === 'v11') source = `https://raw.githubusercontent.com/discordjs/discord.js/docs/${source}.json`;

    const queryString = qs.stringify({ src: source, q: value, force: false, includePrivate: true })
