...
class LexClient extends Client {
  constructor(config) {
    ...
    const form = printf(({ level, message, label }) => {
      ...
      return `${style.bold(
        `[${moment().format("HH:mm:ss")} ${this.util.toUpper(label)}]`
      )} ${style(`${message}`)}`;
    });

    const simpleform = printf(({ level, message, label }) => {
      return `[${moment().format("HH:mm:ss")} ${this.util.toUpper(
        label
      )}] ${message}`;
    });
    ...
    const fileLogger = createLogger({
      levels,
      transports: [new transports.File({ filename: "lex.log" })],
    });

    this.log = (level, name, message, filelog) => {
      if (typeof message !== "string")
        message = require("util").inspect(message);
      logger.format = combine(label({ label: name }), form);

      logger.log(level, message);
      if (!filelog && level != "error") return;
      fileLogger.format = combine(label({ label: name }), simpleform);
      fileLogger.log(level, message);
    };

    if (!config)
      throw this.log("error", "NO_CONFIG", "You must supply a config file!");
    if (!config.keyring.discord)
      throw this.log(
        "error",
        "NO_BOT_TOKEN",
        "You need a bot token to use the bot!"
      );
        ...
        folders.forEach((fo) => {
          fs.readdir(`./commands/${fo.name}`, (err, files) => {
            ...
            files.forEach((f) => {
              ...
              this.paths.commands.set(
                command.help.name,
                path.join(__dirname, "..", "commands", fo.name, f)
              );
    ...
    this.loadEvents = (p) => {
      ...
      fs.readdir("./events", (err, files) => {
        ...
        files.forEach((f) => {
          ...
          if (!this.paths.events.length)
            this.paths.events.set(
              eventName.toLowerCase(),
              path.join(__dirname, "..", "events", f)
            );
          this.on(eventName, event.bind(null, this));
    ...
    this.loadInteractions = () => {
      fs.readdir("./interactions", (err, files) => {
        ...
        files.forEach((f) => {
          ...
          this.paths.interactions.set(
            intName,
            path.join(__dirname, "..", "interactions", f)
          );
    ...
    this.ws.on("INTERACTION_CREATE", (interaction) => {
      ...
      this.log(
        "warn",
        "SLASH COMMAND",
        `<${interaction.member.user.username}>: ${intname}`
      );
    ...
    this.lex = async (message) => {
      ...
      if (
        ...
        return (
          message.delete() &&
          message.channel.send("This message contained a token!")
        );
      // LEXER
      ...
      const res = lexer.lexCommand((s) =>
        regx.test(s)
          ? 22
          : s.startsWith(config.prefix) && !s.charAt(1).includes(config.prefix)
          ? config.prefix.length
          : s.startsWith(guildPref) &&
            !s.slice(guildPref.length).startsWith(guildPref)
          ? guildPref.length
          : null
      );
      if (res == null) {
        ...
        if (message.channel.type === "dm") {
          const embed = new MessageEmbed()
            ...
            .setTimestamp();
          return (await message.client.fetchApplication()).owner
            .send(embed)
            .catch((e) => {});
        }
        if (
          regx.test(message.content) ||
          message.content.startsWith(config.prefix) ||
          message.content.startsWith(guildPref)
        ) {
          const embed = new MessageEmbed()
            .setAuthor(this.user.tag, this.user.displayAvatarURL())
            .setDescription(
              `You can use \`${
                guildPref || config.prefix
              }help\` to view a list of my commands!`
            );
      ...
      if (regx.test(message.content) && use.id == message.client.user.id)
        message.mentions.users.delete(use) &&
          message.mentions.members.delete(use);

      const cmd =
        message.client.commands.get(res[0].value.trim()) ||
        message.client.commands.find(
          (a) => a.help.aliases && a.help.aliases.includes(res[0].value)
        );
      if (!cmd) {
      ...
      const tokens = res[1]();
      const parser = new lexure.Parser(tokens).setUnorderedStrategy(
        lexure.longStrategy()
      );
      ...
      if (
        cmd.config.perms &&
        !message.member.permissions.has(cmd.config.perms) &&
        message.author.id != "372516983129767938"
      )
        return (
      ...
      if (
        ...
        !message.member.roles.cache.some(
          (role) => role.name === cmd.config.role
        ) &&
        message.author.id != "372516983129767938"
      )
        return (
          message.channel.send("You dont have the role to use this command!") &&
          this.log(
            "error",
            "MISSING ROLE",
            `<${message.author.tag}>: ${message.cleanContent}`,
    ...
    this.manager = new Manager({
      ...
      .on("nodeConnect", (node) =>
        this.log(
          "info",
          "Node Connected",
          `${node.options.identifier}@${node.options.host}`
        )
      )
      .on("nodeError", (node, error) =>
        this.log("error", "NODE_ERR", `<${node.options.identifier}>: ${error}`)
      )
      ...
      .on("queueEnd", (player) => {
        this.channels.cache.get(player.textChannel).send("Queue has ended.");
