...
module.exports = {
  ...
  execute(message, args) {
    ...
    if (reloadType == "command" && !reloadName) {
      ...
      try {
        ...
        return message.reply(
          "There was an error trying to reload all commands!"
        );
      }
    ...
    if (reloadType === "command" && reloadName) {
      const command =
        message.client.commands.get(reloadName) ||
        message.client.commands.find(
          (cmd) => cmd.help.aliases && cmd.help.aliases.includes(reloadName)
        );
      if (!command)
        return message.channel.send(
          `There is not command named \`${reloadName}\``
        );
      ...
      try {
        ...
        message.channel.send(
          `Successfully reloaded command \`${command.help.name}\``
        );
      } catch (e) {
        console.log(e);
        message.reply(
          `There was an error wile reload a command \`${command.help.name}\`:\n\`${e.message}\``
        );
      }
    }
    ...
    if (reloadType === "events" && reloadName) {
      ...
      if (!path)
        return message.reply(`There is no event with the name ${reloadName}`);
