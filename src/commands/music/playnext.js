...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const arg = joinTokens(args.many());
    const { channel } = message.member.voice;
    if (!channel) return message.reply("You need to be in a voice channel");
    if (channel.id !== player.voiceChannel)
      return message.reply("You are not in the same channel as the player");
      ...
      switch (res.loadType) {
        ...
        case "TRACK_LOADED":
          ...
          if (!player.playing && !player.paused && !player.queue.size)
            return player.play();
          return message.reply(
            `Adding \`${res.tracks[0].title}\` to the queue.`
          );
        case "SEARCH_RESULT":
          let max = 5,
            collected,
    ...
    const track = player.queue.splice(arg - 1, 1);
