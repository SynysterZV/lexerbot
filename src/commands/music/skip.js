...
module.exports = {
    ...
    execute(message, args) {
        ...
        if(!num) {
            ...
            if (isNaN(num)) return message.reply('You must reply which track NUMBER to skip!');
            song = `${player.queue[num - 1].title} was skipped`;

            player.queue.remove(num - 1);
        }
        else if (num2) {
            if (isNaN(num) || isNaN(num2)) return message.reply('You must specify which track NUMBERS to skip!');
