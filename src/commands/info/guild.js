  ...
  async execute(message, args) {
    ...
    if (n) {
      const embed = new MessageEmbed()
        .setDescription(
          message.client.guilds.cache.map((g) => `• ${g.name}`).join("\n")
        )
    ...
    const tc = message.guild.channels.cache.filter((c) => c.type === "text")
      .size;
    const vc = message.guild.channels.cache.filter((c) => c.type === "voice")
      .size;
    ...
    const embed = new MessageEmbed()
      ...
      .addFields(
        ...
        {
          name: "❯ Members",
          value: `• Humans: ${members}\n• Bots: ${bots}\n• Online: ${online}`,
        },
        {
          name: "❯ Premium",
          value: `• Premium Level: ${message.guild.premiumTier}\n• Boosts: ${message.guild.premiumSubscriptionCount}`,
        },
        {
          name: "❯ Other",
