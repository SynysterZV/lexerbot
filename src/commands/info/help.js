...
module.exports = {
  ...
  async execute(message, args, prefix) {
    ...
    let d = args.single();
    let input = d ? d.toLowerCase() : undefined;

    let categories = [];
    const catfolders = require("fs")
    ...
    if (!input) {
      ...
      const array = message.client.commands
        .filter((c) => c.help.category == input)
        .array();
      const maxPages = Math.ceil(array.length / multiple);
      if (page < 1 || page > maxPages)
        return message.channel.send("That page doesnt exist");
      ...
      for (const i in commands) {
        embed.addField(
          `${prefix}${commands[i].help.name}`,
          commands[i].help.desc
        );
      }
    } else {
      const command =
        message.client.commands.get(input.toLowerCase()) ||
        message.client.commands.find(
          (cmd) =>
            cmd.help.aliases && cmd.help.aliases.includes(input.toLowerCase())
        );
      if (!command)
        return message.channel.send(`There is no command named \`${input}\``);
      ...
      embed.addFields(
        ...
        {
          name: "Aliases:",
          value: command.help.aliases.length ? command.help.aliases : "None",
        },
        {
          name: "Usage",
          value: command.help.usage ? `\`;${command.help.usage}\`` : "None",
        }
      );
    }
