...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const member =
      ...
      (await message.guild.members.fetch(memberArg));
    ...
    if (!timevar || timevar > 1000 * 60 * 60 * 24 * 7)
      return message.reply(
        "Please provide a valid time between 1 second and 7 days"
      );
    const timeString = message.client.timeString(timevar, { verbose: true });

    const roles = member.roles.cache.map((r) => r.id);
    const mutedRole = message.guild.roles.cache.find(
      (r) => r.name === message.client.config.mutedRole
    );

    if (message.client.muted.has(member.id))
    ...
    member.roles.remove(roles).then((m) => {
      ...
      message.client.emit("modEvent", "MUTE", {
        member,
        reason,
        executor,
        time: timeString,
      ...
      setTimeout(async () => {
        ...
        m.roles
          .remove(mutedRole)
          .catch((e) => message.client.log("error", "MUTE_ERR", e));
        m.roles.add(roles);
      }, timevar);
    });
    return message.reply(
      `Successfully muted **${member.user.tag}** for **${reason}** (Time: ${timeString})`
    );
  },
};
