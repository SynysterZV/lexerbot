  ...
  async execute(message, args) {
    ...
    if (!res.list.length) {
      return message.reply(
        "Im sorry, I couldnt find that word on Urban Dictionary!"
      );
    }
    const def = res.list[0];

    if (def.definition.length > 2048)
      return message.reply("This definition is too long!");

    const embed = new MessageEmbed()
      .setAuthor(
        `Urban Dictionary`,
        "https://play-lh.googleusercontent.com/unQjigibyJQvru9rcCOX7UCqyByuf5-h_tLpA-9fYH93uqrRAnZ0J2IummiejMMhi5Ch"
      )
      .setColor("YELLOW")
