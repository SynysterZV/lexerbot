    ...
    async execute(message, args) {
        ...
        if (!repository.includes('#') && !commit) {
            ...
            const embed = new MessageEmbed()
                ...
                .addFields(
                    ...
                    { name: 'Last Push', value: new Date(body.pushed_at).toLocaleString()}
        ...
        try {
            const res = await fetch('https://api.github.com/graphql', {
                method: 'POST',
                headers: { Authorization: `Bearer ${message.client.config.keyring.github}`},
        ...
        if (!body?.data?.repository) {
            return message.channel.send('Im sorry i couldnt find the repo you were looking for');
        }
        ...
        const embed = new MessageEmbed()
			...
			.addField(
				'Labels',
				d.labels.nodes.length ? d.labels.nodes.map((node) => node.name) : 'NO LABEL(S)',
				true,
			)
